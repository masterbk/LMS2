...
//                array('class' => 'outcome clearfix'));
        ...
        if ($qa->get_question(false)->get_type_name() == 'multichoiceset') {
            if ($qa->get_state_class($options->correctness && $qa->has_marks()) == 'incorrect') {
                $output .= '<span style="color:red;">Thiếu câu trả lời đúng. Bạn cần chọn đúng và đủ tất cả các câu trả lời đúng mới được điểm.</span>';
            }
        }
