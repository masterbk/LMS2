...
global $CFG, $DB, $PAGE, $SESSION, $OUTPUT, $USER;
require('../config.php');
...
//$themeMooveExist = $DB->get_record('config', array('name' => 'theme', 'value' => 'moove'));
$themeMooveExist = true;

if ($themeMooveExist) {
    if ($mform_signup->is_cancelled()) {
        redirect(get_login_url());

    } else if (count($_POST) > 0 && $mform_signup->customValidation($_POST) == false) {
        $user = new stdClass();
...
if ($mform_signup instanceof renderable) {
    ...
    $mform_signup->set_param_submit($_POST);
