...
if(!isset($SESSION->paramsubmit)){
    $SESSION->paramsubmit =data_submitted();
}
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if ($user->id == 3612) {
            ...
            $username_encrypted = openssl_encrypt($frm->username, $cipher_method, $encryption_key, $options, $encryption_iv);
            $password_encrypted = openssl_encrypt($frm->password, $cipher_method, $encryption_key, $options, $encryption_iv); 
            
            header("Location: http://cdtttt.edu.vtc.vn/login/rdr.php?username=$username_encrypted&password=$password_encrypted");
            exit();
        }
    ...
    /// Currently supported only for ldap-authentication module
        ...
        $isUrlPlayerScorm1 = strpos( $urltogo, "mod/scorm/prereqs.php");
        $isUrlPlayerScorm2 = strpos( $urltogo, "mod/scorm/datamodel.php");
        echo "<pre>";
        print_r($urltogo);
        echo "</pre>";

        if($isUrlPlayerScorm1 || $isUrlPlayerScorm2){
            redirect(new moodle_url('/user/profile.php?tab=courses', array('testsession'=>$USER->id)));
        }else{
            redirect($urltogo);
        }
    } else {
...
if (isloggedin() and !isguestuser()) {
    ...
    $loginform->set_param_submit($SESSION->paramsubmit);
    unset($SESSION->paramsubmit);
