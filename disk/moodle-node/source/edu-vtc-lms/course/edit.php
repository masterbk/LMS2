...
if ($editform->is_cancelled()) {
    ...
    if (empty($course->id)) {
        ...
        file_save_draft_area_files($data->references, \context_course::instance($course->id)->id, 'local_vtc', 'references', $course->id, ['subdirs' => 0]
        );
...
if (!empty($course->id)) {
    $draftitemid = file_get_submitted_draft_itemid('references');
    file_prepare_draft_area($draftitemid, context_course::instance($course->id)->id, 'local_vtc', 'references', $course->id, ['subdirs' => 0]);
    $editform->set_data(array(
        'references' => $draftitemid
    ));
}
// Print the form.
