...
class core_course_category implements renderable, cacheable_object, IteratorAggregate {
        ...
        if (array_key_exists($name, self::$coursecatfields)) {
            if ($this->$name === false) {
                ...
                $record             = $DB->get_record(
                    'course_categories',
                    array('id' => $this->id),
                    join(',', array_keys($notretrievedfields)),
                    MUST_EXIST
                );
    ...
    public static function create($data, $editoroptions = null) {
        ...
        if ($editoroptions) {
            $newcategory = file_postupdate_standard_editor(
                $newcategory,
                'description',
                $editoroptions,
                $categorycontext,
     ...
     */
    public static function user_enrolment_changed(
        $courseid,
        $userid,
        $status,
        $timestart = null,
        ...
        if (($invisibleids = $coursecatcache->get('ic' . $this->id)) === false) {
            ...
            if ($catids) {
                ...
                $contexts           = $DB->get_records_sql(
                    "SELECT $ctxselect FROM {context} ctx
            ...
            if ($sortfieldskeys[0] === 'sortorder') {
                ...
                if ($invisibleids = $this->get_not_visible_children_ids()) {
                    ...
                    $records            = self::get_records(
                        'cc.parent = :parent AND cc.id ' . $sql,
    ...
    public function has_courses() {
        ...
        return $DB->record_exists_sql(
            "select 1 from {course} where category = ?",
            array($this->id)
        );
    }
    ...
    public function can_delete_full() {
        ...
        foreach ($childcategories as $childcat) {
            ...
            if (
                (!$childcat->visible && !has_capability('moodle/category:viewhiddencategories', $childcontext)) ||
    ...
    protected function change_parent_raw(core_course_category $newparentcat) {
        ...
        $DB->set_field(
            'course_categories',
            'sortorder',
            get_max_courses_in_category() * MAX_COURSE_CATEGORIES,
            ['id' => $this->id]
        );
     ...
     * @return core_course_category|null
     */
    public static function get_nearest_editable_subcategory(
        core_course_category $parentcat,
        array $permissionstocheck
