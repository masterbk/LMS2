...
if ($id) {
    ...
    $showinmenunavbar = (getCustomField("context_coursecat", $category->id, "showinmenunavbar")['showinmenunavbar']);
    ...
    $itemid = null;
    $showinmenunavbar = 1;// Set this explicitly, so files for parent category should not get loaded in draft area.
}
...
$mform = new core_course_editcategory_form(null, array(
    ...
    'context'    => $context,
    'itemid'     => $itemid,
    'showinmenunavbar'=>$showinmenunavbar
));
...
$options = [
    ...
    'subdirs'        => 0,
    'accepted_types' => array('.png', '.jpg', '.gif', '.webp', '.tiff', '.svg')
];
$data = new stdClass();
if($id){
    $mform->set_data(file_prepare_standard_filemanager($data, 'categorypicture', $options, $context, 'course', 'category', 0));
}
...
if ($mform->is_cancelled()) {
    ...
    if (isset($coursecat)) {
        ...
        vtcCustomFieldCategory($data->showinmenunavbar, $context->id, $coursecat->id);
        file_postupdate_standard_filemanager($data, 'categorypicture', $options, $context, 'course', 'category', 0);
    } else {
        $category = core_course_category::create($data, $mform->get_description_editor_options());
        $context = context_coursecat::instance($category->id);
        vtcCustomFieldCategory($data->showinmenunavbar, $context->id, $category->id);
        file_postupdate_standard_filemanager($data, 'categorypicture', $options, $context, 'course', 'category', 0);
