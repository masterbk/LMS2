...
class core_course_external extends external_api {
            ...
            foreach ($sections as $key => $section) {
                ...
                if (empty($filters['excludemodules']) and !empty($modinfosections[$section->section])) {
                    foreach ($modinfosections[$section->section] as $cmid) {
                        ...
                        $module['quiztimelimit']='';
                        $module['quiznumberquestion']=0;
                        $module['quizattempts'] = 0;
                        if($module['modname'] == 'quiz'){
                            $quiz = $DB->get_record('quiz', ['id' => $module['instance']], '*', MUST_EXIST);
                            $total = $DB->count_records('quiz_slots', array('quizid' => $quiz->id));
                            $module['quiztimelimit'] = themeMooveTimeIntToString($quiz->timelimit/60);
                            $module['quiznumberquestion'] = $total;
                            $module['quizattempts'] = $quiz->attempts;
                        }
                        ...
                        $module['time'] = getCustomField('context_course', $courseid, 'time')['time'];
                    ...
                    'modules' => new external_multiple_structure(
                            new external_single_structure(
                                array(
                                    ...
                                    'quiztimelimit' => new external_value(PARAM_TEXT, 'quiz time limit'),
                                    'quiznumberquestion' => new external_value(PARAM_INT, 'quiz number question'),
                                    'quizattempts' => new external_value(PARAM_INT, 'quiz attempt number'),
    ...
    protected static function get_course_public_information(core_course_list_element $course, $coursecontext) {
        ...
        $c = $DB->get_record('course',array('id'=>$course->id));
        ...
        $coursereturns['categoryparentid']  = $DB->get_record('course_categories',array('id'=>$course->category))->parent;
        ...
        $coursereturns['listteacher']  = $teachers;
    ...
    protected static function get_course_structure($onlypublicdata = true) {
        $coursestructure = array(
            ...
            'categoryid' => new external_value(PARAM_INT, 'category id'),
            'categoryparentid'=> new external_value(PARAM_INT, 'category id'),
            ...
            'listteacher'=>new external_multiple_structure(
                new external_single_structure(
                    array(
                        "id"           => new external_value(PARAM_INT, 'user id'),
                        "name"         =>new external_value(PARAM_TEXT, 'user name'),
                        "image"        => new external_value(PARAM_TEXT, 'user image'),
                        "academicRank" =>new external_value(PARAM_TEXT, 'user academic'),
