...
function send_stored_file($stored_file, $lifetime=null, $filter=0, $forcedownload=false, array $options=array()) {
    ...
    if ($filename == 'cdnvtc.mp4') {
        header("Location: https://aea-2041b.kxcdn.com/cdnvtc-scorm12/assets/cdnvtc.mp4"); die();
    }
...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $offline = false, $embed = false) {
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'icon' and $context->contextlevel == CONTEXT_USER) {
            ...
            if ($filename !== 'f1' and $filename !== 'f2' and $filename !== 'f3' and $filename !== 'f0') {
                $filename = 'f1';
            }
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'summary' || $filearea === 'overviewfiles' || $filearea === 'category') {
            if ($CFG->forcelogin) {
                require_login();
            }
            ...
            $filename = array_pop($args);
            $filepath = $args ? '/'.implode('/', $args).'/' : '/';
            $demo =$fs->get_file($contextid, 'course', $filearea, 0, $filepath, $filename);
